...
int main(int argc, char *argv[])
{
    ...
    long int position = -1;
    float peso_idf = 0;
    int freqeuncy = 0; // recebe um int com a frequencia de uma palavra oriunda do comando 'uniq -c' de shell
    tDado palavra; // estrutra que sera gravada no arquivo .bin

    // Read a word from the database file
    while (fscanf(database_file, "%[^:]: %f\n", palavra_banco, &peso_idf) != EOF)
    {
        position++;
        fseek(input_file, 0, SEEK_SET);
        fscanf(input_file, "%*[^\n]");
        // Search for the word in the input file
        while (fscanf(input_file, "%d %s", &freqeuncy, palavra_documento ) != EOF)
        {
            //printf("    palavra %ld : %s\n",position, palavra_documento);
            if (!strcmp(palavra_documento, palavra_banco))
            {
                palavra.frequencia = (float)freqeuncy;
